...
# #the number of shapes we're gonna create in each of the two classes
# nfigs = 4000

# #Specify the size of the image
# #Ex: size = 32 will create images with 32x32 pixels

# size = 32

# #loop over classes

# for someClass in ["squares", "triangles"]:
#      print("generating images of "+someClass+":")
...
#         #set the position and the scale of each figure drawn with random numbers
#         # the coefficients are used to just try and prevent too many shapes from
#         #spilling off the edge of the image
        
#         basex = 0.7*random.random() # x-position
#         basey = 0.7*random.random() #y-position
#         length = 0.5*random.random() # length-scale
        
#         if someClass == "squares": #drawing squares
#             path_data = [
#                 (Path.MOVETO, (basex, basey)),
#                 (Path.LINETO, (basex + length, basey)),
#                 (Path.LINETO, (basex + length, basey + length)),
#                 (Path.LINETO, (basex, basey + length)),
#                 (Path.LINETO, (basex, basey))#,is a comma supposed to be there?
#             ]
        
#         else: #else make triangles
#             path_data = [
#                 (Path.MOVETO, (basex, basey)),
#                 (Path.LINETO, (basex + length, basey)), #draw line across to the right
#                 (Path.LINETO, (basex + length, basey + length)), #draw line up
#                 (Path.LINETO, (basex, basey+length)), #draw line to top of equivalent triangle
#                 (Path.LINETO, (basex, basey))#, possible placed comma // draw line back to base position
#             ]
        
#         # Get path data into the right format for plotting
