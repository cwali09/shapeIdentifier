...
def getImage(filename):
    ...
    features = tf.parse_single_example(
        fullExample,
        features={
            'image/height': tf.FixedLenFeature([], tf.int64), #---------------tf.FixedLenFeature(shape, dtype, default_value)
    ...
    label = features['image/class/label'] #--------grabbing the tensor we parsed earlier in the dictionary
...
if simpleModel:
  ...
  W_fc1 = weight_variable([int((width/4)) * (int(height/4)) * nFeatures2, nNeuronsfc]) #-------------------------MODIFIED
  b_fc1 = bias_variable([nNeuronsfc])
  
  # flatten output from previous layer
  h_pool2_flat = tf.reshape(h_pool2, [-1, int((width/4)) * int((height/4)) * nFeatures2]) #------------------------MODIFIED
